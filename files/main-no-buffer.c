// ADC1_GPIO33_CHANNEL --- Батарея

// PIN_SCL   	22 --- IR Датчик
// PIN_SDA    	21 --- IR Датчик
...
#define iSteps					8 //9 7777		// Количество промежуточных точек при интерполяции по горизонтали/вертикали
...
// #define DEBUG_MODE
static int64_t start, stop; //для отладки
...
void app_main()
{
	...
	console_printf(MsgInfo, "Выделениe ОЗУ для хранения настроек mlx90640\r\n");
	eeMLX90640 = heap_caps_malloc(832 << 1, MALLOC_CAP_8BIT);
    if (!eeMLX90640)
    	FatalErrorMsg("Ошибка выделения ОЗУ для хранения настроек mlx90640, считанных из EEPROM\r\n");

	// printf(" %d KB FREE HEAP\n", heap_caps_get_free_size(MALLOC_CAP_8BIT) / (1024));//7777
	// vTaskDelay(100 / portTICK_RATE_MS);

    console_printf(MsgInfo, "Выделениe ОЗУ для термограммы\r\n");
	TermoImage = heap_caps_malloc(768 << 2, MALLOC_CAP_8BIT);
    if (!TermoImage)
    	FatalErrorMsg("Ошибка выделения ОЗУ для термограммы\r\n");

	// printf(" %d KB FREE HEAP\n", heap_caps_get_free_size(MALLOC_CAP_8BIT) / (1024));//7777
	// vTaskDelay(100 / portTICK_RATE_MS);

    console_printf(MsgInfo, "Выделениe ОЗУ (int16) для термограммы\r\n");
	TermoImage16 = heap_caps_malloc(768 << 1, MALLOC_CAP_8BIT);
    if (!TermoImage16)
    	FatalErrorMsg("Ошибка выделения ОЗУ (int16) для термограммы\r\n");

	// printf(" %d KB FREE HEAP\n", heap_caps_get_free_size(MALLOC_CAP_8BIT) / (1024));//7777
	// vTaskDelay(100 / portTICK_RATE_MS);		

	#if (CALC_MODE == CALC_MODE_INTERPOL)
		console_printf(MsgInfo, "Выделениe ОЗУ (int16) для интерполированной термограммы\r\n");
	//===============
	...
	//===============
		...
		if (!TermoHqImage16)
			FatalErrorMsg("Ошибка выделения ОЗУ (int16) для интерполированной термограммы\r\n");
	#endif

	#ifdef DEBUG_MODE
	stop = esp_timer_get_time(); //debug
	printf(" %i mks Выделение буферов\n", (uint32_t)(stop - start));
	#endif
    ...
    if (result < 0)
    	FatalErrorMsg("Ошибка чтения EEPROM из mlx90640, код ошибки %d\r\n", result);

    console_printf(MsgInfo, " Извлечение параметров mlx90640 из EEPROM\r\n");
    result = MLX90640_ExtractParameters(eeMLX90640, &params);
    if (result < 0)
    	FatalErrorMsg("Ошибка извлечения параметров mlx90640, код ошибки %d\r\n", result);
	...
	#ifdef DEBUG_MODE
	stop = esp_timer_get_time(); //debug
	printf(" %i mks Насторойка mlx90640\n", (uint32_t)(stop - start));
	#endif
...
//7777777777777777777777777777777	
    ...
    while (1)
    {
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i mks Формирование новой шкалы\n", (uint32_t)(stop - start));
		#endif
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i mks Чтение фрейма из сенсора\n", (uint32_t)(stop - start));
		#endif
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i mks Считаем несколько параметров mlx90640\n", (uint32_t)(stop - start));
		#endif
...
// //    	dispcolor_printf_Bg(68, 228, FONTID_6X8M, RGB565(160, 96, 96), BLACK, "BAT=%.0f%% ", capacity);
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i mks Расчёт матрицы температур\n", (uint32_t)(stop - start));
		#endif
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i Копируем температуры в целочисленный массив\n", (uint32_t)(stop - start));
		#endif
			...
			#ifdef DEBUG_MODE
			stop = esp_timer_get_time(); //debug
			printf(" %i Выводим термограму DIRECT\n", (uint32_t)(stop - start));
			#endif
		#endif
		#if (CALC_MODE == CALC_MODE_INTERPOL)
			// Выводим термограму
			...
			#ifdef DEBUG_MODE
			stop = esp_timer_get_time(); //debug
			printf(" %i Интерполируем термограму\n", (uint32_t)(stop - start));
			#endif
			...
			#ifdef DEBUG_MODE
			stop = esp_timer_get_time(); //debug
			printf(" %i Выводим термограму INTERPOL\n", (uint32_t)(stop - start));
			#endif
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i Выводим температуру в центре экрана\n", (uint32_t)(stop - start));
		#endif
		...
		#ifdef DEBUG_MODE
		stop = esp_timer_get_time(); //debug
		printf(" %i Вывод минимальной и максимальной температуры в кадре\n", (uint32_t)(stop - start));
		#endif
